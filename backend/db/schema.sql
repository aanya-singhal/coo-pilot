...
-- Re-running this file on an existing database: add the newer columns and
-- widen the status check without dropping anything.
alter table claims add column if not exists claim_number text;
alter table claims add column if not exists exporter text;
do $$
begin
    alter table claims drop constraint if exists claims_status_check;
    alter table claims add constraint claims_status_check
        check (status in ('CREATED', 'PROCESSING', 'PENDING_REVIEW',
                          'APPROVED', 'REJECTED', 'FAILED', 'REQUESTED_INFO'));
end $$;
...
create table if not exists extracted_data (
    ...
    extraction_status text not null default 'SUCCESS'
                      check (extraction_status in ('SUCCESS', 'FAILED', 'SKIPPED')),
    created_at        timestamptz not null default now()
);
...
-- Human review dispositions. An automated result never closes a case; a
-- reviewer approves, rejects, or asks for more information, and every such
-- act is recorded here alongside the audit log.
create table if not exists decisions (
    id          uuid primary key default gen_random_uuid(),
    claim_id    uuid not null references claims(id) on delete cascade,
    decision    text not null
                check (decision in ('APPROVED', 'REJECTED', 'REQUESTED_INFO')),
    reviewer    text not null,
    comments    text,
    created_at  timestamptz not null default now()
);

create index if not exists decisions_claim_id_idx on decisions (claim_id, created_at);
